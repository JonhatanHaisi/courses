...
from django.views.generic import RedirectView, TemplateView, ListView, DetailView, RedirectView
...
class PagSeguroView(LoginRequiredMixin, RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        ...
        pg = order.pagseguro()
        pg.redirect_url = self.request.build_absolute_uri(reverse('checkout:order_detail', args=[order.pk]))
        pg.notification_url = self.request.build_absolute_uri(reverse('checkout:pagseguro_notification'))
