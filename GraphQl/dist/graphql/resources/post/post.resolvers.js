...
exports.postResolvers = {
    Post: {
        author: (post, args, { db, dataloaders: { userLoader } }, info) => {
            return userLoader.load({ key: post.get('author'), info: info })
        ...
        updatePost: composable_resolver_1.compose(...auth_resolver_1.authResolvers)((parent, { id, input }, { db, authUser }, info) => {
            ...
            return db.sequelize.transaction((t) => {
                return db.Post
                    ...
                    .then((post) => {
                    utils_1.throwError(!post, `Post with id ${id} not found`);
                    utils_1.throwError(post.get('author') != authUser.id, `Unautorized. You can only update pots created by yourself.`);
        ...
        deletePost: composable_resolver_1.compose(...auth_resolver_1.authResolvers)((parent, { id }, { db, authUser }, info) => {
            ...
            return db.sequelize.transaction((t) => {
                return db.Post
                    ...
                    .then((post) => {
                    utils_1.throwError(!post, `Post with id ${id} not found`);
                    utils_1.throwError(post.get('author') != authUser.id, `Unautorized. You can only delete pots created by yourself.`);
